...
class ZenModeInterface(QWidget):
    def __init__(self, parent=None):
        ...
        # Question Content
        self.questionContent = BodyLabel("请认真审题，不要被外界干扰。\n\n已知有一质量为 m 的滑块在倾角为 θ 的斜面上匀速下滑...\n\n求：动摩擦因数 μ = ?", self)
        self.questionContent.setWordWrap(True)
        self.questionContent.setStyleSheet("font-size: 20px; line-height: 1.6;")
        self.layout.addWidget(self.questionContent, stretch=2, alignment=Qt.AlignTop | Qt.AlignHCenter)

        # Drafting Area
        self.draftArea = TextEdit(self)
        self.draftArea.setPlaceholderText("在这里写草稿或代码解答...")
        self.draftArea.setStyleSheet("background-color: #1E1E1E; border: 1px solid #333; color: white; font-family: Consolas;")
